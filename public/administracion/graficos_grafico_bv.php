...
// Create page object
$graficos_grafico_blobview = new cgraficos_grafico_blobview();
$Page =& $graficos_grafico_blobview;

// Page init
$graficos_grafico_blobview->Page_Init();

// Page main
$graficos_grafico_blobview->Page_Main();
?>
<?php
$graficos_grafico_blobview->Page_Terminate();
?>
<?php
...
// Page class
//
class cgraficos_grafico_blobview {

	// Page ID
	var $PageID = 'blobview';

	// Page object name
	var $PageObjName = 'graficos_grafico_blobview';
	...
	//
	function cgraficos_grafico_blobview() {
	...
	function Page_Main() {
		...
		$objBinary = new cUpload('graficos', 'x_grafico');
		...
		if ($rs = $conn->Execute($sSql)) {
			if (!$rs->EOF) {
				...
				if (trim(strval($rs->fields('grafico'))) <> "") {
					header("Content-Disposition: attachment; filename=" . $rs->fields('grafico'));
				}
				$objBinary->Value = $rs->fields('grafico');
