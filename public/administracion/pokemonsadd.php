...
// Create page object
$pokemons_add = new cpokemons_add();
$Page =& $pokemons_add;

// Page init
$pokemons_add->Page_Init();

// Page main
$pokemons_add->Page_Main();
?>
...
// Create page object
var pokemons_add = new ew_Page("pokemons_add");

// page properties
pokemons_add.PageID = "add"; // page ID
pokemons_add.FormID = "fpokemonsadd"; // form ID
var EW_PAGE_ID = pokemons_add.PageID; // for backward compatibility

// extend page with ValidateForm function
pokemons_add.ValidateForm = function(fobj) {
...
pokemons_add.Form_CustomValidate =  
...
pokemons_add.ValidateRequired = true; // uses JavaScript validation
<?php } else { ?>
pokemons_add.ValidateRequired = false; // no JavaScript validation
<?php } ?>
...
<p><span class="phpmaker"><?php echo $Language->Phrase("Add") ?>&nbsp;<?php echo $Language->Phrase("TblTypeTABLE") ?><?php echo $pokemons->TableCaption() ?><br><br>
<a href="<?php echo $pokemons->getReturnUrl() ?>"><?php echo $Language->Phrase("GoBack") ?></a></span></p>
<?php
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
$pokemons_add->ShowMessage();
?>
<form name="fpokemonsadd" id="fpokemonsadd" action="<?php echo ew_CurrentPage() ?>" method="post" enctype="multipart/form-data" onsubmit="return pokemons_add.ValidateForm(this);">
<p>
...
class cpokemons_add {
	...
	function Page_Main() {
		...
		if (@$_POST["a_add"] <> "") {
		   $pokemons->CurrentAction = $_POST["a_add"]; // Get form action
		  $this->GetUploadFiles(); // Get upload files
		  $this->LoadFormValues(); // Load form values

			// Validate form
			if (!$this->ValidateForm()) {
				$pokemons->CurrentAction = "I"; // Form error, reset action
				$this->setMessage($gsFormError);
			}
		} else { // Not post back
		  if ($bCopy) {
		    $pokemons->CurrentAction = "C"; // Copy record
		  } else {
		    $pokemons->CurrentAction = "I"; // Display blank record
		...
		switch ($pokemons->CurrentAction) {
		  ...
		  case "A": // ' Add new record
				$pokemons->SendEmail = TRUE; // Send email on add success
		...
		$pokemons->RowType = EW_ROWTYPE_ADD;  // Render add type
	...
	function LoadDefaultValues() {
		global $pokemons;
		$pokemons->imagen->CurrentValue = NULL; // Clear file related field
		$pokemons->icono->CurrentValue = NULL; // Clear file related field
	}
	...
	function RenderRow() {
		...
		} elseif ($pokemons->RowType == EW_ROWTYPE_ADD) { // Add row
	...
	function AddRow() {
		...
		$bInsertRow = $pokemons->Row_Inserting($rsnew);
		if ($bInsertRow) {
			...
			$AddRow = $conn->Execute($pokemons->InsertSQL($rsnew));
			$conn->raiseErrorFn = '';
		} else {
		...
		if ($AddRow) {
			$pokemons->id->setDbValue($conn->Insert_ID());
			$rsnew['id'] = $pokemons->id->DbValue;

			// Call Row Inserted event
			$pokemons->Row_Inserted($rsnew);
		}
