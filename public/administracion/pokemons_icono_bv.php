...
// Create page object
$pokemons_icono_blobview = new cpokemons_icono_blobview();
$Page =& $pokemons_icono_blobview;

// Page init
$pokemons_icono_blobview->Page_Init();

// Page main
$pokemons_icono_blobview->Page_Main();
?>
<?php
$pokemons_icono_blobview->Page_Terminate();
?>
<?php
...
// Page class
//
class cpokemons_icono_blobview {

	// Page ID
	var $PageID = 'blobview';

	// Page object name
	var $PageObjName = 'pokemons_icono_blobview';
	...
	//
	function cpokemons_icono_blobview() {
	...
	function Page_Main() {
		...
		$objBinary = new cUpload('pokemons', 'x_icono');

		// Show thumbnail
		$bShowThumbnail = (@$_GET["showthumbnail"] == "1");
		if (@$_GET["thumbnailwidth"] == "" && @$_GET["thumbnailheight"] == "") {
			$iThumbnailWidth = 32; // Set default width
			$iThumbnailHeight = 32; // Set default height
		} else {
		...
		if ($rs = $conn->Execute($sSql)) {
			if (!$rs->EOF) {
				...
				if (trim(strval($rs->fields('icono'))) <> "") {
					header("Content-Disposition: attachment; filename=" . $rs->fields('icono'));
				}
				$objBinary->Value = $rs->fields('icono');
