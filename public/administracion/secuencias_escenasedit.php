...
// Create page object
$secuencias_escenas_edit = new csecuencias_escenas_edit();
$Page =& $secuencias_escenas_edit;

// Page init
$secuencias_escenas_edit->Page_Init();

// Page main
$secuencias_escenas_edit->Page_Main();
?>
...
// Create page object
var secuencias_escenas_edit = new ew_Page("secuencias_escenas_edit");

// page properties
secuencias_escenas_edit.PageID = "edit"; // page ID
secuencias_escenas_edit.FormID = "fsecuencias_escenasedit"; // form ID
var EW_PAGE_ID = secuencias_escenas_edit.PageID; // for backward compatibility

// extend page with ValidateForm function
secuencias_escenas_edit.ValidateForm = function(fobj) {
...
secuencias_escenas_edit.Form_CustomValidate =  
...
secuencias_escenas_edit.ValidateRequired = true; // uses JavaScript validation
<?php } else { ?>
secuencias_escenas_edit.ValidateRequired = false; // no JavaScript validation
<?php } ?>
...
<p><span class="phpmaker"><?php echo $Language->Phrase("Edit") ?>&nbsp;<?php echo $Language->Phrase("TblTypeTABLE") ?><?php echo $secuencias_escenas->TableCaption() ?><br><br>
<a href="<?php echo $secuencias_escenas->getReturnUrl() ?>"><?php echo $Language->Phrase("GoBack") ?></a></span></p>
<?php
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
$secuencias_escenas_edit->ShowMessage();
?>
<form name="fsecuencias_escenasedit" id="fsecuencias_escenasedit" action="<?php echo ew_CurrentPage() ?>" method="post" enctype="multipart/form-data" onsubmit="return secuencias_escenas_edit.ValidateForm(this);">
<p>
<input type="hidden" name="a_table" id="a_table" value="secuencias_escenas">
...
<?php if ($secuencias_escenas->id->Visible) { // id ?>
	<tr<?php echo $secuencias_escenas->id->RowAttributes ?>>
		...
		<td<?php echo $secuencias_escenas->id->CellAttributes() ?>><span id="el_id">
<div<?php echo $secuencias_escenas->id->ViewAttributes() ?>><?php echo $secuencias_escenas->id->EditValue ?></div><input type="hidden" name="x_id" id="x_id" value="<?php echo ew_HtmlEncode($secuencias_escenas->id->CurrentValue) ?>">
</span><?php echo $secuencias_escenas->id->CustomMsg ?></td>
	</tr>
<?php } ?>
...
<a href="<?php echo $secuencias_escenas->imagen->HrefValue ?>"><?php echo $secuencias_escenas->imagen->EditValue ?></a>
<?php } elseif (!in_array($secuencias_escenas->CurrentAction, array("I", "edit", "gridedit"))) { ?>	
&nbsp;
<?php } ?>
<?php } else { ?>
<?php if (!empty($secuencias_escenas->imagen->Upload->DbValue)) { ?>
<?php echo $secuencias_escenas->imagen->EditValue ?>
<?php } elseif (!in_array($secuencias_escenas->CurrentAction, array("I", "edit", "gridedit"))) { ?>	
&nbsp;
<?php } ?>
<?php } ?>
...
<?php $secuencias_escenas->imagen->EditAttrs["onchange"] = "this.form.a_imagen[2].checked=true;" . @$secuencias_escenas->imagen->EditAttrs["onchange"]; ?>
<?php } else { ?>
...
<?php
$secuencias_escenas_edit->Page_Terminate();
?>
<?php
...
// Page class
//
class csecuencias_escenas_edit {
	...
	// Page object name
	var $PageObjName = 'secuencias_escenas_edit';
	...
	//
	function csecuencias_escenas_edit() {
	...
	function Page_Main() {
		...
		if (@$_POST["a_edit"] <> "") {
			$secuencias_escenas->CurrentAction = $_POST["a_edit"]; // Get action code
			$this->GetUploadFiles(); // Get upload files
			$this->LoadFormValues(); // Get form values

			// Validate form
			if (!$this->ValidateForm()) {
				$secuencias_escenas->CurrentAction = ""; // Form error, reset action
				$this->setMessage($gsFormError);
				$secuencias_escenas->EventCancelled = TRUE; // Event cancelled
				$this->RestoreFormValues();
			}
		} else {
			$secuencias_escenas->CurrentAction = "I"; // Default action is display
		}

		// Check if valid key
		if ($secuencias_escenas->id->CurrentValue == "")
			$this->Page_Terminate("secuencias_escenaslist.php"); // Invalid key, return to list
		switch ($secuencias_escenas->CurrentAction) {
			case "I": // Get a record to display
				...
				break;
			Case "U": // Update
				$secuencias_escenas->SendEmail = TRUE; // Send email on update success
		...
		// Render the record
		$secuencias_escenas->RowType = EW_ROWTYPE_EDIT; // Render as Edit
		$this->RenderRow();
	}
	...
	function RenderRow() {
		...
		if ($secuencias_escenas->RowType == EW_ROWTYPE_VIEW) { // View row
		...
		} elseif ($secuencias_escenas->RowType == EW_ROWTYPE_EDIT) { // Edit row

			// id
			$secuencias_escenas->id->EditCustomAttributes = "";
			$secuencias_escenas->id->EditValue = $secuencias_escenas->id->CurrentValue;
			$secuencias_escenas->id->CssStyle = "";
			$secuencias_escenas->id->CssClass = "";
			$secuencias_escenas->id->ViewCustomAttributes = "";
			...
			// imagen
			if (!ew_Empty($secuencias_escenas->imagen->Upload->DbValue)) {
				$secuencias_escenas->imagen->HrefValue = ew_UploadPathEx(FALSE, $secuencias_escenas->imagen->UploadPath) . ((!empty($secuencias_escenas->imagen->EditValue)) ? $secuencias_escenas->imagen->EditValue : $secuencias_escenas->imagen->CurrentValue);
				if ($secuencias_escenas->Export <> "") $secuencias_escenas->imagen->HrefValue = ew_ConvertFullUrl($secuencias_escenas->imagen->HrefValue);
			} else {
				$secuencias_escenas->imagen->HrefValue = "";
			}
	...
	function EditRow() {
		...
		if ($rs->EOF) {
			...
			$secuencias_escenas->imagen->Upload->DbValue = $rs->fields('imagen'); // Get original value
			if (is_null($secuencias_escenas->imagen->Upload->Value)) {
				$rsnew['imagen'] = NULL;
			} else {
				if ($secuencias_escenas->imagen->Upload->FileName == $secuencias_escenas->imagen->Upload->DbValue) { // Upload file name same as old file name
					$rsnew['imagen'] = $secuencias_escenas->imagen->Upload->FileName;
				} else {
			...
			// orden
			$secuencias_escenas->orden->SetDbValueDef($rsnew, $secuencias_escenas->orden->CurrentValue, 0, FALSE);

			// Call Row Updating event
			$bUpdateRow = $secuencias_escenas->Row_Updating($rsold, $rsnew);
			if ($bUpdateRow) {
			...
			if ($secuencias_escenas->imagen->Upload->Action == "2" || $secuencias_escenas->imagen->Upload->Action == "3") { // Update/Remove
				...
				$conn->raiseErrorFn = 'ew_ErrorFn';
				$EditRow = $conn->Execute($secuencias_escenas->UpdateSQL($rsnew));
				$conn->raiseErrorFn = '';
			} else {
		...
		if ($EditRow)
			$secuencias_escenas->Row_Updated($rsold, $rsnew);
		$rs->Close();
