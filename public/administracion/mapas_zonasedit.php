...
// Create page object
$mapas_zonas_edit = new cmapas_zonas_edit();
$Page =& $mapas_zonas_edit;

// Page init
$mapas_zonas_edit->Page_Init();

// Page main
$mapas_zonas_edit->Page_Main();
?>
...
// Create page object
var mapas_zonas_edit = new ew_Page("mapas_zonas_edit");

// page properties
mapas_zonas_edit.PageID = "edit"; // page ID
mapas_zonas_edit.FormID = "fmapas_zonasedit"; // form ID
var EW_PAGE_ID = mapas_zonas_edit.PageID; // for backward compatibility

// extend page with ValidateForm function
mapas_zonas_edit.ValidateForm = function(fobj) {
...
mapas_zonas_edit.Form_CustomValidate =  
...
mapas_zonas_edit.ValidateRequired = true; // uses JavaScript validation
<?php } else { ?>
mapas_zonas_edit.ValidateRequired = false; // no JavaScript validation
<?php } ?>
...
<p><span class="phpmaker"><?php echo $Language->Phrase("Edit") ?>&nbsp;<?php echo $Language->Phrase("TblTypeTABLE") ?><?php echo $mapas_zonas->TableCaption() ?><br><br>
<a href="<?php echo $mapas_zonas->getReturnUrl() ?>"><?php echo $Language->Phrase("GoBack") ?></a></span></p>
<?php
if (EW_DEBUG_ENABLED)
	echo ew_DebugMsg();
$mapas_zonas_edit->ShowMessage();
?>
<form name="fmapas_zonasedit" id="fmapas_zonasedit" action="<?php echo ew_CurrentPage() ?>" method="post" onsubmit="return mapas_zonas_edit.ValidateForm(this);">
<p>
<input type="hidden" name="a_table" id="a_table" value="mapas_zonas">
...
<?php if ($mapas_zonas->id->Visible) { // id ?>
	<tr<?php echo $mapas_zonas->id->RowAttributes ?>>
		...
		<td<?php echo $mapas_zonas->id->CellAttributes() ?>><span id="el_id">
<div<?php echo $mapas_zonas->id->ViewAttributes() ?>><?php echo $mapas_zonas->id->EditValue ?></div><input type="hidden" name="x_id" id="x_id" value="<?php echo ew_HtmlEncode($mapas_zonas->id->CurrentValue) ?>">
</span><?php echo $mapas_zonas->id->CustomMsg ?></td>
	</tr>
<?php } ?>
...
<?php
$mapas_zonas_edit->Page_Terminate();
?>
<?php
...
class cmapas_zonas_edit {
	...
	// Page object name
	var $PageObjName = 'mapas_zonas_edit';
	...
	function Page_Main() {
		...
		if (@$_POST["a_edit"] <> "") {
			$mapas_zonas->CurrentAction = $_POST["a_edit"]; // Get action code
			$this->LoadFormValues(); // Get form values

			// Validate form
			if (!$this->ValidateForm()) {
				$mapas_zonas->CurrentAction = ""; // Form error, reset action
				$this->setMessage($gsFormError);
				$mapas_zonas->EventCancelled = TRUE; // Event cancelled
				$this->RestoreFormValues();
			}
		} else {
			$mapas_zonas->CurrentAction = "I"; // Default action is display
		}

		// Check if valid key
		if ($mapas_zonas->id->CurrentValue == "")
			$this->Page_Terminate("mapas_zonaslist.php"); // Invalid key, return to list
		switch ($mapas_zonas->CurrentAction) {
			case "I": // Get a record to display
				...
				break;
			Case "U": // Update
				$mapas_zonas->SendEmail = TRUE; // Send email on update success
		...
		// Render the record
		$mapas_zonas->RowType = EW_ROWTYPE_EDIT; // Render as Edit
		$this->RenderRow();
	}
	...
	function RenderRow() {
		...
		} elseif ($mapas_zonas->RowType == EW_ROWTYPE_EDIT) { // Edit row

			// id
			$mapas_zonas->id->EditCustomAttributes = "";
			$mapas_zonas->id->EditValue = $mapas_zonas->id->CurrentValue;
			$mapas_zonas->id->CssStyle = "";
			$mapas_zonas->id->CssClass = "";
			$mapas_zonas->id->ViewCustomAttributes = "";
